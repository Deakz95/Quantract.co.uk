...
import { Mail, Lock, Sparkles, ArrowLeft, ChevronRight, CheckCircle2, AlertCircle, Users } from "lucide-react";
...
export default function ClientLogin() {
    ...
    try {
      if (mode === "magic") {
        const res = await fetch("/api/auth/magic-link/request", {
          ...
          body: JSON.stringify({ role: "client", email, rememberMe }),
        });
        ...
        const res = await fetch("/api/auth/password/login", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ role: "client", email, password, rememberMe }),
        });
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-emerald-500 rounded-full blur-3xl opacity-10 animate-float" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-teal-500 rounded-full blur-3xl opacity-10 animate-float" style={{ animationDelay: '1s' }} />

      <div className="relative w-full max-w-md animate-fade-in">
        {/* Logo */}
        <div className="text-center mb-8">
          <div className="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center shadow-xl mb-4">
            <Users className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-2xl font-bold text-[var(--foreground)]">Welcome Back</h1>
          <p className="text-[var(--muted-foreground)] mt-1">Sign in to your client portal</p>
        </div>

        <Card variant="glass" className="backdrop-blur-xl">
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              <Badge className="bg-gradient-to-r from-emerald-500 to-teal-500 text-white border-0">Client Portal</Badge>
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-[var(--border)] text-emerald-500 focus:ring-emerald-500 cursor-pointer"
                />
