...
import { Mail, Lock, Sparkles, ArrowLeft, ChevronRight, CheckCircle2, AlertCircle, Settings } from "lucide-react";
...
export default function EngineerLogin() {
    ...
    try {
      if (mode === "magic") {
        const res = await fetch("/api/auth/magic-link/request", {
          ...
          body: JSON.stringify({ role: "engineer", email, rememberMe }),
        });
        ...
        const res = await fetch("/api/auth/password/login", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ role: "engineer", email, password, rememberMe }),
        });
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-orange-500 rounded-full blur-3xl opacity-10 animate-float" />
      <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-amber-500 rounded-full blur-3xl opacity-10 animate-float" style={{ animationDelay: '1s' }} />

      <div className="relative w-full max-w-md animate-fade-in">
        {/* Logo */}
        <div className="text-center mb-8">
          <div className="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-br from-orange-500 to-amber-500 flex items-center justify-center shadow-xl mb-4">
            <Settings className="w-8 h-8 text-white" />
          </div>
          <h1 className="text-2xl font-bold text-[var(--foreground)]">Welcome Back</h1>
          <p className="text-[var(--muted-foreground)] mt-1">Sign in to your engineer portal</p>
        </div>

        <Card variant="glass" className="backdrop-blur-xl">
          <CardHeader className="pb-4">
            <div className="flex items-center justify-between">
              <Badge className="bg-gradient-to-r from-orange-500 to-amber-500 text-white border-0">Engineer Portal</Badge>
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-[var(--border)] text-orange-500 focus:ring-orange-500 cursor-pointer"
                />
              ...
              {sentTo && (
                <div className="rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-4 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-emerald-600">Check your inbox!</p>
