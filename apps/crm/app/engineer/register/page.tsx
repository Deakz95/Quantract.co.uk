  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (data.invite.role !== "engineer") throw new Error("This link is not for an engineer account.");
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (loginRes.ok) {
        ...
        router.push("/engineer/login?registered=true&email=" + encodeURIComponent(email));
      } else {
        // Login API failed, but registration succeeded - still redirect to login
        router.push("/engineer/login?message=Registration%20complete!%20Please%20log%20in.");
      }
    } catch (e: any) {
