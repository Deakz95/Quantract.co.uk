...
export default function EngineerProfileClient() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      }>(`/api/admin/engineers/${engineerId}/profile`);
      setEngineer(res.engineer);
      setQualifications(res.qualifications);
      setForm({
        name: res.engineer.name ?? "",
        phone: res.engineer.phone ?? "",
        address1: res.engineer.address1 ?? "",
        address2: res.engineer.address2 ?? "",
        city: res.engineer.city ?? "",
        county: res.engineer.county ?? "",
        postcode: res.engineer.postcode ?? "",
        country: res.engineer.country ?? "",
        emergencyName: res.engineer.emergencyName ?? "",
        emergencyPhone: res.engineer.emergencyPhone ?? "",
        emergencyRelationship: res.engineer.emergencyRelationship ?? "",
      });
    } catch (err) {
  ...
  const handleSaveProfile = async () => {
    setSaving(true);
    try {
      await apiRequest(`/api/admin/engineers/${engineerId}/profile`, {
        method: "PATCH",
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await apiRequest<{ ok: boolean; avatarUrl: string }>(
        `/api/admin/engineers/${engineerId}/avatar`,
        { method: "PUT", body: fd },
      );
      setEngineer((prev) => (prev ? { ...prev, avatarUrl: res.avatarUrl } : prev));
  ...
  const handleAddQualification = async () => {
    ...
    try {
      ...
      await apiRequest(`/api/admin/engineers/${engineerId}/qualifications`, {
        method: "POST",
        body: fd,
      });
      toast({ title: "Qualification added", variant: "success" });
      setShowQualForm(false);
      setQualForm({ name: "", type: "", issuer: "", certificateNumber: "", issueDate: "", expiryDate: "", notes: "" });
      setQualFile(null);
      await load();
    } catch (err) {
      toast({ title: "Failed to add", description: getApiErrorMessage(err), variant: "destructive" });
    } finally {
      setSavingQual(false);
    }
  };
  ...
  const handleDeleteQualification = async (qualId: string) => {
    if (!confirm("Delete this qualification? This action cannot be undone.")) return;
    try {
      await apiRequest(`/api/admin/engineers/${engineerId}/qualifications/${qualId}`, {
        method: "DELETE",
      });
  ...
  if (loading) return <div className="space-y-3">{Array.from({ length: 8 }).map((_, i) => <LoadingSkeleton key={i} />)}</div>;
  if (error) return <ErrorState description={error} onRetry={load} />;
  if (!engineer) return <ErrorState title="Engineer not found" />;
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card>
            <CardContent className="pt-6 flex flex-col items-center text-center gap-4">
              ...
              <div>
                <h2 className="text-lg font-semibold">{engineer.name || "Unnamed"}</h2>
                <p className="text-sm text-[var(--muted-foreground)]">{engineer.email}</p>
              </div>
              ...
              {engineer.rateCard && (
                <p className="text-xs text-[var(--muted-foreground)]">
                  Rate Card: {engineer.rateCard.name}
                </p>
              )}
              {uploadingAvatar && (
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card>
              ...
              <CardContent>
                <div className="grid gap-4 sm:grid-cols-2">
                  <FieldInput label="Full Name" value={form.name} onChange={(v) => setForm({ ...form, name: v })} />
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid gap-4 sm:grid-cols-2">
                  <FieldInput label="Address Line 1" value={form.address1} onChange={(v) => setForm({ ...form, address1: v })} />
                  <FieldInput label="Address Line 2" value={form.address2} onChange={(v) => setForm({ ...form, address2: v })} />
                  <FieldInput label="City" value={form.city} onChange={(v) => setForm({ ...form, city: v })} />
                  <FieldInput label="County" value={form.county} onChange={(v) => setForm({ ...form, county: v })} />
                  <FieldInput label="Postcode" value={form.postcode} onChange={(v) => setForm({ ...form, postcode: v })} />
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid gap-4 sm:grid-cols-2">
                  <FieldInput label="Contact Name" value={form.emergencyName} onChange={(v) => setForm({ ...form, emergencyName: v })} />
                  <FieldInput label="Contact Phone" value={form.emergencyPhone} onChange={(v) => setForm({ ...form, emergencyPhone: v })} type="tel" />
                  <FieldInput label="Relationship" value={form.emergencyRelationship} onChange={(v) => setForm({ ...form, emergencyRelationship: v })} />
                </div>
              </CardContent>
            </Card>
      ...
      {tab === "certificates" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Button size="sm" onClick={() => setShowQualForm(!showQualForm)}>
          ...
          {showQualForm && (
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-4 sm:grid-cols-2">
                  <FieldInput label="Qualification Name *" value={qualForm.name} onChange={(v) => setQualForm({ ...qualForm, name: v })} placeholder="e.g. 18th Edition" />
                  <FieldInput label="Type" value={qualForm.type} onChange={(v) => setQualForm({ ...qualForm, type: v })} placeholder="e.g. electrical, gas" />
                  <FieldInput label="Issuer" value={qualForm.issuer} onChange={(v) => setQualForm({ ...qualForm, issuer: v })} placeholder="e.g. City & Guilds" />
                  <FieldInput label="Certificate Number" value={qualForm.certificateNumber} onChange={(v) => setQualForm({ ...qualForm, certificateNumber: v })} />
                  <FieldInput label="Issue Date" value={qualForm.issueDate} onChange={(v) => setQualForm({ ...qualForm, issueDate: v })} type="date" />
                  <FieldInput label="Expiry Date" value={qualForm.expiryDate} onChange={(v) => setQualForm({ ...qualForm, expiryDate: v })} type="date" />
                </div>
                <FieldInput label="Notes" value={qualForm.notes} onChange={(v) => setQualForm({ ...qualForm, notes: v })} />
                <div>
                  ...
                  />
                  <p className="mt-1 text-xs text-[var(--muted-foreground)]">PDF, JPEG, PNG, WebP (max 10MB)</p>
                </div>
                <div className="flex gap-2 justify-end">
                  <Button variant="ghost" size="sm" onClick={() => { setShowQualForm(false); setQualFile(null); }}>
                    Cancel
                  </Button>
                  <Button size="sm" onClick={handleAddQualification} disabled={savingQual || !qualForm.name.trim()}>
          ...
          {qualifications.length === 0 ? (
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="text-sm text-[var(--muted-foreground)]">No qualifications added yet</p>
              </CardContent>
            </Card>
          ) : (
            <Card>
                        ...
                        <tr key={q.id} className="border-b border-[var(--border)] last:border-0">
                          <td className="px-4 py-3">
                            <div>
                              ...
                              {q.type && (
                                <Badge variant="outline" className="ml-2 text-xs">{q.type}</Badge>
                              )}
                            </div>
                          </td>
                          <td className="px-4 py-3 text-[var(--muted-foreground)]">{q.issuer || "—"}</td>
                          <td className="px-4 py-3 text-[var(--muted-foreground)]">{q.certificateNumber || "—"}</td>
                          <td className="px-4 py-3 text-[var(--muted-foreground)]">
                            {q.issueDate ? new Date(q.issueDate).toLocaleDateString("en-GB") : "—"}
                          </td>
                          <td className="px-4 py-3 text-[var(--muted-foreground)]">
                            {q.expiryDate ? new Date(q.expiryDate).toLocaleDateString("en-GB") : "—"}
                          </td>
                          <td className="px-4 py-3">{expiryBadge(q.expiryDate)}</td>
