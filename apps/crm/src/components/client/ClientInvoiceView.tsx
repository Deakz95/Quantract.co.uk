      ...
      <Card>
        <CardHeader>
          <div className="flex items-start justify-between gap-3">
            <div>
              <CardTitle>Invoice</CardTitle>
              <div className="mt-1 text-xs text-slate-600">Invoice ID: {inv.id}</div>
            </div>
        ...
        <CardContent>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-4">
            <div className="text-xs font-semibold text-slate-600">Billed To</div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mt-1 text-sm font-semibold text-slate-900">{pounds(inv.subtotal)}</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="text-xs font-semibold text-slate-600">VAT</div>
              <div className="mt-1 text-sm font-semibold text-slate-900">{pounds(inv.vat)}</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div className="text-xs font-semibold text-slate-600">Total</div>
              <div className="mt-1 text-sm font-semibold text-slate-900">{pounds(inv.total)}</div>
            </div>
          </div>

          <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <div className="text-xs font-semibold text-slate-600">Paid so far</div>
              <div className="mt-1 text-sm font-semibold text-slate-900">{pounds(totalPaid)}</div>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-white p-4">
              <div className="text-xs font-semibold text-slate-600">Balance due</div>
              <div className="mt-1 text-sm font-semibold text-slate-900">{pounds(balanceDue)}</div>
            </div>
          </div>
          ...
          {!isPaid && canPay ? (
            <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
              <div className="text-sm font-semibold text-slate-900">Pay invoice</div>
              <div className="mt-1 text-xs text-slate-600">
                We’ll open a secure payment page in a new tab and return you here once it’s complete.
              </div>
              <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center">
                <div className="flex-1">
                  <label className="block text-xs font-semibold text-slate-600">Amount (GBP)</label>
                  <input
                  ...
                  />
                  <div className="mt-1 text-xs text-slate-500">You can pay a partial amount. Balance updates automatically.</div>
                </div>
          ...
          {summary?.payments?.length ? (
            <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
              <div className="text-xs font-semibold text-slate-600">Payments</div>
              <div className="mt-2 space-y-2">
                {summary.payments.map((p) => (
                  <div key={p.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-100 p-3 text-sm">
                    <div>
                      <div className="font-semibold text-slate-900">{pounds(p.amount)}</div>
                      <div className="text-xs text-slate-600">{new Date(p.receivedAtISO).toLocaleString("en-GB")} • {p.provider.toUpperCase()} • {p.status}</div>
                    </div>
