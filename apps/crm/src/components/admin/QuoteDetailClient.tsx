        ...
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              ...
              <div className="mt-3">
                <div className="text-xs font-semibold text-slate-700">Attach existing client</div>
                <select
                ...
                <div className="mt-1 text-xs text-slate-500">Selecting a client will overwrite name/email and site address from the client record.</div>
              </div>
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-3">
            <div className="text-xs font-semibold text-slate-700">Client link</div>
            <div className="mt-1 flex flex-col gap-2 sm:flex-row sm:items-center">
              ...
              <a className="text-sm font-semibold text-slate-900 underline" href={quote.shareUrl} target="_blank" rel="noreferrer">
                Open
              </a>
            </div>
          ...
          {quote.agreement && quote.status !== "accepted" && (
            <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-3">
              <div className="text-xs font-semibold text-slate-700">Agreement link</div>
              <div className="mt-1 flex flex-col gap-2 sm:flex-row sm:items-center">
                <input
                  className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"
                  readOnly
                ...
                <a href={`/api/client/agreements/${quote.agreement.shareUrl.split("/").pop()}/pdf`} target="_blank" rel="noreferrer" className="text-sm font-semibold text-slate-900 underline">
                  PDF
                </a>
                {quote.agreement.status === "signed" && (
                  <a href={`/client/agreements/${quote.agreement.shareUrl.split("/").pop()}/certificate`} target="_blank" rel="noreferrer" className="text-sm font-semibold text-slate-900 underline">
                    Certificate
                  </a>
                )}
              </div>
              <div className="mt-2 text-xs text-slate-600">
                Agreement status: <span className="font-semibold text-slate-900">{quote.agreement.status}</span>
              </div>
            </div>
          )}
          ...
          {quote.audit ? (
            <div className="mt-6">
              <div className="text-xs font-semibold text-slate-700">Audit trail</div>
                    ...
                    {[...quote.audit.quote, ...quote.audit.agreement]
                      ...
                      .map((e) => (
                        <tr key={e.id} className="border-t border-slate-100">
                          <td className="py-3 px-3 text-xs text-slate-600">{new Date(e.createdAtISO).toLocaleString()}</td>
                          <td className="py-3 px-3 font-semibold text-slate-900">{e.action}</td>
                          <td className="py-3 px-3 text-xs text-slate-600">{e.actorRole}{e.actor ? ` • ${e.actor}` : ""}</td>
                        </tr>
                      ))}
